...
object Day6 {
  def enigma1(input: List[Int], numberDay: Int): Int = {
    val fishes = List.range(0, numberDay).foldLeft(input.toVector) {
      (acc, num) => {
        acc.flatMap(item =>
          if (item == 0) Vector(6, 8) else Vector(item - 1)
        )
      }
    }
  ...
  def main(args: Array[String]) {
    val toolAOC = new ToolAOC("src/main/resources/Edition2021/Day6.txt")
    val input = toolAOC.getInputfile().head.split(",").toList.map(_.toInt)

    // Enigma 1
    val result1 = enigma1(input,80)
    println(s"$result1")
